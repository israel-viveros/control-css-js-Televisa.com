	...
	jQuery.mbMiniPlayer = {
		...
		buildPlayer: function (options) {
			...
			this.each(function (idx) {
				...
				var $volumeBox = jQuery("<span/>").addClass("map_volume").html('<i class="tvsa-volume-min tvsa-1x"></i>');
				...
				var $playBox = jQuery("<span/>").addClass("map_play").html('<i class="tvsa-play"></i>');
					...
					ready  : function () {
						...
						function animatePlayer(anim) {
							...
							if (!player.isOpen) {
								...
								$('.hide_control_player .map_controls').animate({width: 1}, speed, function () {
						...
						$playBox.on(jQuery.mbMiniPlayer.eventEnd, function (e) {
							...
							if (!player.isOpen) {
								...
								jQuery(this).html('<i class="tvsa-pause"></i>');
								...
								jQuery(this).html('<i class="tvsa-play"></i>');
						...
						$volumeBox.on(jQuery.mbMiniPlayer.eventEnd,
								function () {
									if (jQuery(this).hasClass("mute")) {
										...
										jQuery(this).html('<i class="tvsa-volume-min tvsa-1x"></i>');
										el.jPlayer("volume", player.opt.volume);
									} else {
										jQuery(this).addClass("mute");
										jQuery(this).html('<i class="tvsa-volume-mute tvsa-1x"></i>');
										el.jPlayer("volume", 0);
									}
								}).hover(
								function () {
